...
function buildTimelineData(): Array<{ date: string; score: number }> {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    data.push({ date: iso, score: scores[29 - i] })
  }
  return data
}
    ...
    <div>
      ...
      >
              ...
              >
                ...
                <div className="mt-2.5 flex flex-wrap items-center gap-3 text-[11.5px]">
                  <span className="text-ink-3">
                    Confidence:{' '}
                    <span className="font-medium text-ink">{fmtConfidence(alert.confidence)}</span>
                  </span>
                  <span className="text-ink-3">
                    Cause:{' '}
        ...
        <div className="border-t border-line px-4 py-2.5 text-[11.5px] leading-relaxed text-ink-3">
          Alerts are generated by the ensemble model every 15 minutes. Critical alerts page an
          analyst immediately; warn-level alerts are batched for the morning review.
        </div>
      </Panel>
        ...
        <DataTable
          ...
          columns={[
            ...
            },
            {
              ...
              align: 'right',
              cell: (r) => (
                <span className="vm-num font-medium text-ink">{fmtPct(r.precision)}</span>
              ),
            },
            {
              key: 'recall',
              ...
              align: 'right',
              cell: (r) => (
                <span className="vm-num font-medium text-ink">{fmtPct(r.recall)}</span>
              ),
            },
            {
              key: 'f1',
            ...
            },
            {
              ...
              align: 'right',
              cell: (r) => (
                <span className="vm-num text-ink-2">{fmtInt(r.latencyMs)} ms</span>
              ),
            },
            {
              key: 'status',
      ...
      <div className="mt-3">
        <Callout tone="neutral" title="How to interpret these results">
          Every figure on this page is simulated for demonstration. The ensemble model, thresholds
          and rule logic described here reflect the intended production design. Backtesting on a
          held-out 30-day window from the 90-day training panel yields the precision and recall
          figures shown above. Critical alerts require analyst acknowledgement before the index cell
          is suppressed.
        </Callout>
      </div>
    </div>

    ...
    <div className={`card overflow-hidden`}>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between">
          <div>
            <Link to={`/animals/${alert.animalId}`} className="text-base font-semibold text-gray-900 hover:text-brand-700">
              {alert.animalId}
            </Link>
        ...
        <div className="mt-3 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600">
          <span className="font-medium text-gray-500">{t('alerts.predWindow')}: </span>
          {t('common.days')}
        ...
        return (
          <div key={f.key || f.label}>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">{f.label || f.key}</span>
              <span className="font-medium text-gray-900">{f.delta}</span>
            </div>
      ...
      {items.map((it, i) => {
        ...
        return (
          <li key={i} className="relative">
            <span className={`absolute -left-[31px] grid h-4 w-4 place-items-center rounded-full ${tone.dot} ring-4 ring-white`} />
            <div className="flex items-center gap-2">
              <Icon size={14} className="text-gray-400" />
              <span className="text-xs font-medium text-gray-400">{it.date}</span>
            </div>
            <p className="mt-0.5 text-sm text-gray-700">{it.label}</p>
          </li>
        )
      })}
    </ol>
  ...
  return (
    <div className="card overflow-hidden">
            ...
            {animals.map((a) => (
              <tr key={a.id} className="hover:bg-gray-50/70">
                <td className="px-4 py-3">
                  <Link to={`/animals/${a.id}`} className="font-medium text-gray-900 hover:text-brand-700">{a.id}</Link>
                  <div className="text-xs text-gray-400">{a.name} · {a.species}</div>
                </td>
                <td className="px-4 py-3 text-gray-600">{a.breed}</td>
                <td className="px-4 py-3 text-gray-600">{a.age}y</td>
                <td className="px-4 py-3 text-gray-600">{a.lactation}</td>
                <td className="px-4 py-3 text-gray-600">{a.milkYield} L</td>
                <td className="px-4 py-3 text-gray-600">{a.scc}k</td>
                <td className={`px-4 py-3 ${a.activity < 0 ? 'text-red-600' : 'text-gray-600'}`}>{a.activity > 0 ? '+' : ''}{a.activity}%</td>
      ...
      <div className="divide-y divide-gray-100 md:hidden">
        {animals.map((a) => (
          <Link key={a.id} to={`/animals/${a.id}`} className="flex items-center gap-3 p-4 active:bg-gray-50">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
  ...
  return (
    <div className={`rounded-xl border p-4 ${m.border} ${m.bg}`}>
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-gray-900">{shed.name}</span>
      ...
      <p className="mt-1 text-xs text-gray-500">{shed.animals} animals</p>
    </div>
  )
}
        ...
        <tbody className="divide-y divide-gray-100">
          {animals.map((a) => {
            ...
            return (
              <tr key={a.id}>
                <td className="py-2.5">
                  <Link to={`/animals/${a.id}`} className="font-semibold text-gray-900 hover:text-brand-700">{a.id}</Link>
                </td>
                <td className="py-2.5 text-gray-500">{a.species}</td>
                <td className="py-2.5">
    ...
    <div className="relative overflow-hidden rounded-xl border border-gray-200">
      ...
      {SHEDS.map((s) => {
        ...
        return (
          <div
            key={s.id}
            className="absolute -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white/95 px-2 py-1 text-[11px] font-semibold shadow-md ring-1 ring-black/5"
            style={SHED_POS[s.id]}
          >
      ...
      <div className="absolute bottom-2 left-2 flex flex-wrap gap-2 rounded-lg bg-white/95 px-2 py-1 text-[10px] shadow ring-1 ring-black/5">
      ...
      <span className="absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-lg bg-white/95 text-gray-500 shadow ring-1 ring-black/5">
        <Maximize2 size={13} />
      </span>
    </div>
  )
...
export function UrgentAlertItem({ alert }) {
  ...
  const m = riskMeta(alert.level)
  return (
    <Link to={`/animals/${alert.animalId}`} className="block border-t border-gray-100 py-3 first:border-t-0 hover:bg-gray-50/60">
      <div className="flex items-center justify-between">
        <span className="flex items-center gap-2">
          <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-bold uppercase ${m.bg} ${m.text}`}>
            {t(`risk.${alert.level}`)}
          </span>
          <span className="font-semibold text-gray-900">{alert.animalId}</span>
        </span>
        <span className="text-xs font-semibold" style={{ color: m.hex }}>{alert.risk}% {t('dash.risk')}</span>
      </div>
      <p className="mt-1 text-xs text-gray-500">{alert.factors.join(', ')}</p>
    </Link>
  )
}
...
export function QuickAction({ icon: Icon, label, to }) {
  return (
    <Link
      to={to}
      className="flex items-center gap-3 rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:border-brand-300 hover:bg-brand-50/50"
    >
